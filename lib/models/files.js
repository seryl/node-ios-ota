...
(function() {
  var Files, RedisObject, async, filemd5, fs, logger, mkdirp, mv, path, rimraf,
  ...
  Files = (function(_super) {
    ...
    */
    ...
    Files.prototype.setup_file = function(file, fn) {
      ...
      return mkdirp([config.get('repository'), dirloc].join('/'), function(err, made) {
        if (err) {
          logger.error("Error setting up directories for `" + _this.current + "`.");
        }
