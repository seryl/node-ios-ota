...
(function() {
  var Files, RedisObject, async, filemd5, fs, mkdirp, mv, path, rimraf,
  ...
  Files = (function(_super) {
    ...
    */
    ...
    Files.prototype.filepath = function(filename) {
      ...
      dirloc = [this.user, this.application, this.dtype.split('.').join('/'), this.current].join('/');
    ...
    */
    ...
    Files.prototype.setup_file = function(file, fn) {
      ...
      return mkdirp([this.config.get('repository'), dirloc].join('/'), function(err, made) {
        if (err) {
          _this.logger.error("Error setting up directories for `" + _this.current + "`.");
        }
