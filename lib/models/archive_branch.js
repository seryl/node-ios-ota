// Generated by CoffeeScript 1.4.0
(function() {
  var ArchiveBranch, Files, RedisObject, async, fs,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

  fs = require('fs');

  async = require('async');

  RedisObject = require('./redis_object');

  Files = require('./files');

  /**
   * A helper for working with archives for a branch.
  */


  ArchiveBranch = (function(_super) {

    __extends(ArchiveBranch, _super);

    function ArchiveBranch(user, application, branch, ref) {
      this.user = user;
      this.application = application;
      this.branch = branch != null ? branch : null;
      this.delete_directories = __bind(this.delete_directories, this);

      this.setup_directories = __bind(this.setup_directories, this);

      this.files = __bind(this.files, this);

      this.delete_all = __bind(this.delete_all, this);

      this["delete"] = __bind(this["delete"], this);

      this.save = __bind(this.save, this);

      this.all = __bind(this.all, this);

      this.find = __bind(this.find, this);

      this.list = __bind(this.list, this);

      this.archivelist_prefix = __bind(this.archivelist_prefix, this);

      ArchiveBranch.__super__.constructor.call(this, ref);
      this.basename = "node-ios-ota::applications";
      this.object_name = 'archive';
    }

    /**
     * Returns the the prefix for the branchlist.
     * @return {String} The archivelist prefix for the branch
    */


    ArchiveBranch.prototype.archivelist_prefix = function() {
      return [this.basename, this.user, this.application, this.object_name, this.branch].join('::');
    };

    /**
     * Returns the list of archives for the current branch.
     * @param {Function} (fn) The callback function
    */


    ArchiveBranch.prototype.list = function(fn) {
      return this.redis.smembers(this.archivelist_prefix(), fn);
    };

    /**
     * Returns the information for the current branch archive.
     * @param {String} (name) The name of the ref to retrieve
     * @param {Function} (fn) The callback function
    */


    ArchiveBranch.prototype.find = function(name, fn) {
      var original,
        _this = this;
      original = this.current;
      this.current = name;
      return this.files().all(function(err, reply) {
        _this.current = original;
        return fn(err, {
          name: name,
          files: reply
        });
      });
    };

    /**
     * Returns the information for all the current application branches.
     * @param {Function} (fn) The callback function
    */


    ArchiveBranch.prototype.all = function(fn) {
      var _this = this;
      return this.list(function(err, archives) {
        return async.map(archives, _this.find, function(err, results) {
          return fn(err, {
            archives: results
          });
        });
      });
    };

    /**
     * Inserts a new archive into the given application.
     * @param {String} (archive) The name of the archive to add
     * @param {Function} (fn) The callback function
    */


    ArchiveBranch.prototype.save = function(fn) {
      var stat_add, status,
        _this = this;
      stat_add = this.redis.sadd(this.archivelist_prefix(), this.current);
      status = stat_add ? null : {
        message: "Error saving archive: `" + this.user + "/" + this.application + "/" + this.current + "`."
      };
      return this.setup_directories(this.current, function(err, reply) {
        return fn(status, _this.current);
      });
    };

    /**
     * Deletes a single archive for the given branch.
     * @param {String} (ref) The archive reference tag
     * @param {Function} (fn) The callback function
    */


    ArchiveBranch.prototype["delete"] = function(ref, fn) {
      var _this = this;
      this.current = branch;
      this.redis.srem(this.archivelist_prefix(), ref);
      return this.files().delete_all(function(err, reply) {
        return _this.delete_directories(branch, function(err, reply) {
          return fn(null, true);
        });
      });
    };

    /**
     * Deletes the branches for a given application.
     * @param {Function} (fn) The callback function
    */


    ArchiveBranch.prototype.delete_all = function(fn) {
      var _this = this;
      return this.list(function(err, branchlist) {
        return async.forEach(branchlist, _this["delete"], fn);
      });
    };

    /**
     * Returns the list of files for the current branch.
     * @return {Object} The Files object for the current branch
    */


    ArchiveBranch.prototype.files = function() {
      return new Files(this.user, this.application, this.object_name, this.current);
    };

    /**
     * Creates the directories for the branch.
     * @param {Object} (branch) The branch to create directories for
     * @param {Function} (fn) The callback function
    */


    ArchiveBranch.prototype.setup_directories = function(branch, fn) {
      var dirloc, target,
        _this = this;
      dirloc = [this.user, this.application, this.object_name, branch].join('/');
      target = [this.config.get('repository'), dirloc].join('/');
      return fs.exists(target, function(exists) {
        if (!exists) {
          return fs.mkdir(target, function(err, made) {
            if (err) {
              _this.logger.error("Error setting up directories for `" + dirloc + "`.");
            }
            return fn(err, made);
          });
        } else {
          return fn(null, false);
        }
      });
    };

    /**
     * Deletes the directories for the branch.
     * @param {Object} (branch) The branch to create directories for
     * @param {Function} (fn) The callback function
    */


    ArchiveBranch.prototype.delete_directories = function(branch, fn) {
      var dirloc,
        _this = this;
      dirloc = [this.user, this.application, this.object_name, branch].join('/');
      return fs.rmdir([this.config.get('repository'), dirloc].join('/'), function(err) {
        if (err) {
          _this.logger.error("Error removing directories for `" + dirloc + "`.");
        }
        return fn(null, true);
      });
    };

    return ArchiveBranch;

  })(RedisObject);

  module.exports = ApplicationBranch;

}).call(this);
