...
(function() {
  var User, WebServer, async, config, errorHandler, express, fs, generate_identity, http, identity, logger, plist, _ref,
            ...
            if (config.get('archive')) {
              ...
              return _this.plist_bundle_version(plist_file['location'], function(err, ref) {
                ...
                return async.map(a_normal, copy_archive_file, function(err, results) {
                  var nplist;
                  nplist = (results.filter(function(X) {
                    return /\.plist/.test(X['name']);
                  })).pop();
                  return _this.archive_plist_update(nplist['location'], function(err, data) {
                    var archive;
